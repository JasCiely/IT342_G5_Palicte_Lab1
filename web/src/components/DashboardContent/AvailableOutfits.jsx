...
const AvailableOutfits = ({ isGuest = false, onUpgrade }) => {
  ...
  const handleBookClick = (outfit) => {
    if (isGuest) {
      if (onUpgrade) {
        onUpgrade();
      } else {
        alert('Please sign in or create an account to book this outfit.');
      }
    } else {
      // Handle authenticated booking
      alert(`Booking ${outfit.name}...`);
    }
  };

  const handleQuickView = (outfit) => {
    // Quick view works for both guest and authenticated users
    alert(`Quick view for: ${outfit.name}\n\nThis would open a modal with more details, sizing info, and availability calendar.`);
  };
      ...
      <div className="available-outfits-content">
        ...
        <div className="outfits-grid">
          {visibleOutfits.map(outfit => (
            <article key={outfit.id} className={`outfit-card ${isGuest ? 'guest-card' : ''}`}>
              <div className="image-container">
                ...
                <button 
                  ...
                  onClick={() => handleQuickView(outfit)}
                >
                  Quick View
                </button>
              </div>
              ...
              <div className="outfit-info">
                ...
                <div className="outfit-footer">
                  ...
                  <button 
                    className={`btn-book ${isGuest ? 'guest-btn' : ''}`}
                    onClick={() => handleBookClick(outfit)}
                  >
                    {isGuest ? (
                      <>
